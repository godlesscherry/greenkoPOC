...
public class JdbcTelemetryRepository implements TelemetryRepository {
  ...
  public List<MetricPoint> queryMetrics(
    ...
    Object[] params;
    if (deviceId.isPresent()) {
      params = new Object[] {from, to, deviceId.get()};
    } else {
  ...
  public List<TimeSeriesPoint> loadPowerSeries(
    ...
    StringBuilder sql = new StringBuilder("SELECT ").append(timeColumn).append(" AS sample_time, ");
    if (deviceId.isPresent()) {
      sql.append(
          "power_kw AS power FROM telemetry WHERE device_id = ? AND "
              + timeColumn
              + " >= ? AND "
              + timeColumn
              + " <= ? ORDER BY "
              + timeColumn);
      return jdbcTemplate.query(
          sql.toString(), new Object[] {deviceId.get(), from, to}, this::mapTimeSeriesPoint);
    }
    sql.append(
        "SUM(power_kw) AS power FROM telemetry WHERE "
            + timeColumn
            + " >= ? AND "
            + timeColumn
            + " <= ? GROUP BY "
            ...
            + " ORDER BY "
            + timeColumn);
    return jdbcTemplate.query(sql.toString(), new Object[] {from, to}, this::mapTimeSeriesPoint);
  }
  ...
  private TimeSeriesPoint mapTimeSeriesPoint(ResultSet rs, int rowNum) throws SQLException {
    return new TimeSeriesPoint(rs.getTimestamp("sample_time").toInstant(), rs.getDouble("power"));
  }
}
